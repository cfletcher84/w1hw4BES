...
def merge_sort(lst):
    if len(lst) > 1:
        halfway = len(lst) // 2
        left_half = lst[:halfway]
        right_half = lst[halfway:]
...
@app.route('/')
def index():
    return 'Welcome to the the Videos Database!'
...
def search_videos():
    ...
    if not search:
        return {'error': "You must have a search quary paramter."}, 400
    video = binary_search(videos, search)
    if video:
        return video
    else:
        return {'error':f"No video with the title {search} in the list."}, 404
